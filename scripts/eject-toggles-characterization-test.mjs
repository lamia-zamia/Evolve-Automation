...
const selectorLengths = new Map([
  ...
  ["#ejectIron", 1],
  ["#ejectCopper", 1],
  ["#resEjector .ea-eject-toggle", 1],
  ["#script_eject_top_row", 1],
]);
...
hooks.setEjectTogglesTestContext({
  EjectManager: { priorityList: [{ id: "Iron" }, { id: "Copper" }] },
  settingsRaw: { res_ejectIron: true },
...
assert.equal(
  trace.some(
    (entry) =>
      entry.kind === "append" &&
      entry.label === "#eject" &&
      entry.content.includes("Auto Eject"),
  ),
  true,
);

trace.length = 0;
hooks.ejectToggles.removeEjectToggles();
assert.deepEqual(
  trace.filter((entry) => entry.kind === "remove").map((entry) => entry.label),
  ["#resEjector .ea-eject-toggle", "#script_eject_top_row"],
);

console.log("Eject toggles bundled characterization tests passed");
